...
for split in splits:
	for category in class_cat:
		class_dir = datadir + '/' + split + '/' + category
		print class_dir
		...
		for seq in temp:
			...
			for i in range(counter):
				im = seq + '/%d.jpg'%(i+1)
				im_ = misc.imresize(misc.imread(im), (224,224))
				im__ = (im_.astype(float32))/255
				features = vgg.relu7.eval(feed_dict={images: [im__], train_mode: False}).squeeze().tolist()
				fvec.append(features)
			csvname = seq+'/'+'vgg19.csv'
			numpy.savetxt(csvname, numpy.array(fvec), delimiter=',')

...
for seqi = 1:44
    ...
    fullfile(fullfile(data_dir, sprintf(pre_seq, seqi)), 'resnet50.txt')
    resnet_arr = csvread(fullfile(fullfile(data_dir, sprintf(pre_seq, seqi)), 'resnet50.txt'));
...
%             size(feat_vec, 1)
            if size(feat_vec, 1) < 10 && size(feat_vec, 1) > 0
                padlen = 10-size(feat_vec, 1);
                padsize = size(feat_vec, 2);
                temp = resnet_arr(t,:);
                feat_vec = vertcat(feat_vec, zeros(padlen, padsize));
                feat_vec = [temp(:)', feat_vec(:)'];
%                 display('padding')
            elseif size(feat_vec, 1) > 10
                temp = resnet_arr(t,:);
                feat_vec = feat_vec(1:10, :);
                feat_vec = [temp(:)', feat_vec(:)'];
%                 display('trimming')
            else
%                 display('skipping')

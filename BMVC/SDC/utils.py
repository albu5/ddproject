...
def get_feature_vector(desc1, desc2):
    ...
    # print(np.abs(v1[0] - v2[0]))
    # print(np.sqrt(s1 * s2))
    feat_vec.append(np.abs(v1[0] - v2[0]) / np.sqrt(s1 * s2 + 1e-8))
    feat_vec.append(np.abs(v1[1] - v2[1]) / np.sqrt(s1 * s2 + 1e-8))
    feat_vec.append(np.abs(v1[2] - v2[2]) / np.sqrt(s1 * s2 + 1e-8))
    feat_vec.append(np.abs(v1[3] - v2[3]) / np.sqrt(s1 * s2 + 1e-8))
    ...
    # print(poly_i.area)
    # ax.clear
    # plot_patch(ax, poly1, 'r')
    # plot_patch(ax, poly2, 'g')
    # plot_patch(ax, poly_i, 'b')
    # print(ax.patches)
    # plt.pause(1)
    feat_vec.append(poly_i.area / np.sqrt(s1 * s2 + 1e-8))
    feat_vec.append(np.abs(action1-action2))

    return feat_vec


def plot_patch(axes, ob, c):
    try:
        patch = PolygonPatch(ob, facecolor=c, edgecolor='b', alpha=0.5, zorder=100, visible=True)
        axes.add_patch(patch)
    except:
        print('polygon not displayed since empty')

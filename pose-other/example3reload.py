...
trainlabels = genfromtxt('MULTIVIEW' + '/' + 'train' + '/labels.txt', delimiter=',')
NUM_LABELS = int(trainlabels.max())

# building placeholders
x = tf.placeholder(tf.float32, shape = [None, 100*40])
...
x_image = tf.reshape(x, [-1,100,40,1])
...
h_pool2_flattened = tf.reshape(h_pool2, [-1, 25*10*64])
...
#sess.run(tf.initialize_all_variables());
tfsaver.restore(sess, "./tfmodels/dev-10100.ckpt")

for i in range(MAX_STEPS):
	batch = read_train('MULTIVIEW', BATCH_SIZE, NUM_LABELS)
	...
	if (i%100 == 0)&(i>0):
		...
		tmpstr = ("/home/ashish/Desktop/ped-det/ncetis/agutils/pose-cnn/tfmodels/dev-%d.ckpt"%(i))
		print tmpstr
		save_path = tfsaver.save(sess, tmpstr)

		batch = read_valid('MULTIVIEW', NUM_LABELS)
